 ...
 */
export interface PrivyEvmDelegatedEmbeddedWalletConfig
  extends PrivyWalletConfig {
  /** The ID of the delegated wallet */
  walletId: string;

  /** The network ID to connect to (e.g., "katana-network") */
  networkId?: string;
...
export class PrivyEvmDelegatedEmbeddedWalletProvider extends WalletProvider {
   ...
   */
  private constructor(
    config: PrivyEvmDelegatedEmbeddedWalletConfig & { address: string }
  ) {
    super();
    ...
    const networkId = config.networkId || "katana-network";
  ...
  public static async configureWithWallet(
    ...
    try {
      if (!config.walletId) {
        throw new Error(
          "walletId is required for PrivyEvmDelegatedEmbeddedWalletProvider"
        );
      }
      ...
      if (embeddedWallets.length === 0) {
        throw new Error(
          `Could not find wallet address for wallet ID ${config.walletId}`
        );
      }
      ...
      if (error instanceof Error) {
        throw new Error(
          `Failed to configure Privy embedded wallet provider: ${error.message}`
        );
      }
  ...
  async signTransaction(transaction: TransactionRequest): Promise<Hex> {
    ...
    try {
      const response = await this.executePrivyRequest<
        PrivyResponse<{ signed_transaction: Hex }>
      >(body);
  ...
  private generatePrivySignature(url: string, body: object): string {
    try {
      ...
      if (error instanceof Error) {
        throw new Error(
          `Error generating Privy authorization signature: ${error.message}`
        );
      }
   ...
   */
  private async executePrivyRequest<T>(
